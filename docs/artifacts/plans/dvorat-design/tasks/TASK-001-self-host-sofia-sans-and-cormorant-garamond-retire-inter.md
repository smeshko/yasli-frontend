...
### GREEN
- [ ] Fetch Sofia Sans and Cormorant Garamond variable woff2 (SIL OFL), subset
      to `latin` + `cyrillic`, drop `latin-ext`/`greek`/`vietnamese`
...
Keep `font-synthesis: none` (BaseLayout.astro:146). With a variable font the
browser must never fake a weight.
